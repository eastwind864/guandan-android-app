...
from .env_utils import Environment, action_vector
    ...
    for device in device_iterator:
        ...
        for position in positions:
            ...
            specs = dict(
                ...
                obs_z=dict(size=(T, 8, 83), dtype=torch.int8),
            )
                ...
                if env_output['done']:
                    for p in positions:
                        ...
                        if diff > 0:
                            ...
                            episode_return_buf[p].extend([0.0 for _ in range(diff-1)]) # 过程中每一步的回报为0
                            ...
                            if len(obs_x_no_action_buf[p]) != len(target_buf[p]):
